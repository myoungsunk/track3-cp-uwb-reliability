# 구현 명세: load_sparam_table.m / build_sim_data_from_table.m
...
| 원본 | 변환 후 |
|------|--------|
| `x_coord [n]` | `x_coord_n_` |
| `y_coord [n]` | `y_coord_n_` |
| `Freq [GHz]` | `Freq_GHz_` |
...
> **PLACEHOLDER**: 실제 변환 이름은 MATLAB 버전에 따라 다를 수 있음.
> `T = readtable('cp_caseA.csv'); disp(T.Properties.VariableNames)` 로 확인 후
> `params.col_*` 기본값을 실제 이름으로 업데이트.
...
```
pol_type = freq_table.pol_type(1)    % 모든 행이 동일
case_id  = freq_table.case_id(1)

if isKey(params.case_label_map, case_id)
    label_val = params.case_label_map(case_id)   % true=LoS, false=NLoS
    labels = repmat(logical(label_val), N_pos, 1)
else
    labels = true(N_pos, 1)
    warning('[build_sim_data] case_id "%s"에 대한 레이블 매핑 없음. LoS로 가정.', case_id)
end
```
